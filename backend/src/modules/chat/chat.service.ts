...
export class ChatService {
  ...
  static async sendMessage(userId: string, sessionId: string, message: string, language?: string) {
    // 1. Validate session
    const session = await prisma.chatSession.findFirst({
      where: { id: sessionId, userId },
      include: { messages: { orderBy: { createdAt: 'asc' } } },
    ...
    const activeLanguage = language || session.language;
    const langName = langMap[activeLanguage] || 'English';
...
CRITICAL REQUIREMENT: You MUST respond EXCLUSIVELY and ENTIRELY in the following language: ${langName}. 
Do NOT reply in English unless specifically requested.

FORMAT REQUIREMENT:
Keep your response short and optimal. When a user asks about a scheme, strictly provide only the following information in bullet points:
- Scheme Name
- Eligibility
- Last Date to Apply (Deadline)
- Important Documents

Do not make it too big. Keep it very concise.
